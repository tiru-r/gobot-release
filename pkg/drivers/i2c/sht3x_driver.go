...
func NewSHT3xDriver(c Connector, options ...func(Config)) *SHT3xDriver {
	// CRC-8/SENSIRION with polynomial 0x31, init 0xff, no reflection, no xor
	s := &SHT3xDriver{
		Driver:   NewDriver(c, "SHT3x", SHT3xAddressA),
		Units:    "C",
		crcTable: makeCRC8TableWithInit(0x31, 0xff),
	}
...
func (s *SHT3xDriver) sendCommandDelayGetResponse(send []byte, delay *time.Duration, expect int) ([]uint16, error) {
	...
	for i := 0; i < expect; i++ {
		crc := crc8ChecksumWithInit(buf[i*3:i*3+2], s.crcTable, 0xff)
		if buf[i*3+2] != crc {
...
// crc8ChecksumWithInit calculates CRC8 checksum for the given data using the lookup table with init value
func crc8ChecksumWithInit(data []byte, table []byte, init byte) byte {
	crc := init
	for _, b := range data {
		crc = table[crc^b]
	}
