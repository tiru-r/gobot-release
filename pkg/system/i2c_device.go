...
// Important note for "os.ModeExclusive": this is not supported without creating the file for character devices, this means
...
// CloseWithContext closes the device with timeout support.
func (d *i2cDevice) CloseWithContext(ctx context.Context) error {
	...
	case <-time.After(5 * time.Second):
		return fmt.Errorf("timeout closing i2c device %s", d.location)
	}
}

// ReadByte reads a byte from the current register of an i2c device.
func (d *i2cDevice) ReadByte() (byte, error) {
	return d.ReadByteWithContext(context.Background())
}

// ReadByteWithContext reads a byte from the current register with context support.
func (d *i2cDevice) ReadByteWithContext(ctx context.Context) (byte, error) {
	d.mutex.Lock()
	defer d.mutex.Unlock()
	...
	err := d.smbusAccess(d.lastAddress, I2C_SMBUS_READ, 0, I2C_SMBUS_BYTE, unsafe.Pointer(&data))
	return data, err
}
...
func (d *i2cDevice) WriteByte(val byte) error {
	...
	return d.smbusAccess(d.lastAddress, I2C_SMBUS_WRITE, val, I2C_SMBUS_BYTE, nil)
}
...
func (d *i2cDevice) openFileLazy(sender string) error { //nolint:unparam // useful for debugging
	// lazy initialization
	// note: "os.ModeExclusive" is not supported without creating the file. This means for the existing character device,
